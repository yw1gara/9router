...
import PoolFitnessTab from "./PoolFitnessTab";
...
export default function ProxyPoolsPage() {
  ...
  useEffect(() => {
    // Defer so the setState inside the loader never runs synchronously
    // during the effect (cascading-render lint rule).
    const t = setTimeout(() => fetchProxyPools(), 0);
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-1 sm:gap-6 sm:px-0">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="text-xs text-text-muted mt-1">
            Egress pool registry — routing, health, and scoped fitness cooldowns.
          </p>
        </div>
      ...
      {/* Tabs */}
      <div className="flex items-center gap-1 rounded-xl border border-black/10 p-1 dark:border-white/10">
        <button
          onClick={() => setActiveTab("pools")}
          className={`flex flex-1 items-center justify-center gap-2 rounded-lg px-3 py-2 text-sm transition-colors sm:flex-none sm:px-4 ${
            activeTab === "pools"
        ...
        >
          ...
          Pools
          <span className={`rounded-full px-1.5 text-[10px] ${activeTab === "pools" ? "bg-white/20" : "bg-black/10 dark:bg-white/10"}`}>
            {proxyPools.length}
          </span>
        </button>
        <button
      ...
      <Card>
        {activeTab === "pools" ? (
          ...
          <PoolFitnessTab pools={proxyPools} notify={notify} />
        )}
      </Card>
